...
export default function SimpleBackdrop() {
  ...
  const themes = useSelector((state) => state.themes.loading);

  React.useEffect(() => {
    if (loading || season || seasonArchive || anime || episodes || themes)
      setOpen(true);
    else setOpen(false);
  }, [loading, season, seasonArchive, anime, episodes, themes]);
